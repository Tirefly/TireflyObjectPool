...
struct FK2Node_ActorPool_SpawnActor_Helper
{
	...
	static FName InstigatorPinName;
...
FName FK2Node_ActorPool_SpawnActor_Helper::InstigatorPinName(TEXT("Instigator"));
...
FName FK2Node_ActorPool_SpawnActor_Helper::SpawnEvenIfCollidingPinName(TEXT("SpawnEvenIfColliding"));
FName FK2Node_ActorPool_SpawnActor_Helper::NoCollisionFailPinName(TEXT("bNoCollisionFail"));


#define LOCTEXT_NAMESPACE "TireflyActorPool"
...
void UK2Node_ActorPool_SpawnActor::AllocateDefaultPins()
{
	...
	// Transform pin
	UEdGraphPin* TransformPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, TBaseStructure<FTransform>::Get(), FK2Node_ActorPool_SpawnActor_Helper::SpawnTransformPinName);
	...
	// Instigator pin
	UEdGraphPin* InstigatorPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, APawn::StaticClass(), FK2Node_ActorPool_SpawnActor_Helper::InstigatorPinName);
	...
	if (TitleType != ENodeTitleType::MenuTitle)
	{
		if (UEdGraphPin* ClassPin = GetClassPin())
		{
			if (ClassPin->LinkedTo.Num() > 0)
			{
				...
				NodeTitle = NSLOCTEXT("K2Node", "ActorPool_SpawnActor_Title_Unknown", "Actor Pool - Spawn Actor");
			}
			else if (ClassPin->DefaultObject == nullptr)
			{
				NodeTitle = NSLOCTEXT("K2Node", "ActorPool_SpawnActor_Title_NONE", "Actor Pool - Spawn Actor - NONE");
			}
			else
			{
				if (CachedNodeTitle.IsOutOfDate(this))
				{
					...
					CachedNodeTitle.SetCachedText(FText::Format(NSLOCTEXT("K2Node", "ActorPool_SpawnActor_Title_Class", "Actor Pool - Spawn Actor - {ClassName}"), Args), this);
				}
...
void UK2Node_ActorPool_SpawnActor::GetPinHoverText(const UEdGraphPin& Pin, FString& HoverTextOut) const
{
	...
	if (UEdGraphPin* InstigatorPin = GetInstigatorPin())
	{
		K2Schema->ConstructBasicPinTooltip(*InstigatorPin,
			LOCTEXT("InstigatorPinDescription", "Can be left empty; primarily used for gameplay event like damage taken."),
			InstigatorPin->PinToolTip);
	}
...
void UK2Node_ActorPool_SpawnActor::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	static const FName BeginSpawningBlueprintFuncName = GET_FUNCTION_NAME_CHECKED(UTireflyActorPoolLibrary, GenericActorPool_BeginDeferredActorSpawn);
	static const FName FinishSpawningFuncName = GET_FUNCTION_NAME_CHECKED(UTireflyActorPoolLibrary, GenericActorPool_FinishSpawningActor);
	...
	UEdGraphPin* TheInstigatorPin = TheNode->GetInstigatorPin();
...
#pragma region BeginSpawn
	...
	CallBeginTheNode->FunctionReference.SetExternalMember(BeginSpawningBlueprintFuncName, UTireflyActorPoolLibrary::StaticClass());
	...
	UEdGraphPin* CallBeginInstigatorPin = CallBeginTheNode->FindPinChecked(FK2Node_ActorPool_SpawnActor_Helper::InstigatorPinName);
	...
	if (TheInstigatorPin != nullptr)
	{
		CompilerContext.MovePinLinksToIntermediate(*TheInstigatorPin, *CallBeginInstigatorPin);
	}
...
#pragma region FinishSpawn
	...
	CallFinishTheNode->FunctionReference.SetExternalMember(FinishSpawningFuncName, UTireflyActorPoolLibrary::StaticClass());
...
{
	return LOCTEXT("PoolCategory", "TireflyObjectPool");
}
...
bool UK2Node_ActorPool_SpawnActor::IsSpawnVarPin(UEdGraphPin* Pin) const
{
	...
	return (Pin->PinName != K2Schema->PN_Then &&
		...
		Pin->PinName != FK2Node_ActorPool_SpawnActor_Helper::InstigatorPinName &&
...
UEdGraphPin* UK2Node_ActorPool_SpawnActor::GetInstigatorPin() const
{
	UEdGraphPin* Pin = FindPin(FK2Node_ActorPool_SpawnActor_Helper::InstigatorPinName);
	check(Pin == nullptr || Pin->Direction == EGPD_Input);
	return Pin;
}
